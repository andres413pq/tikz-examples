...
absolute coordinates. The anchors ``current page.north west`` and ``current page.south east``
...
Unfortunately the ``current page`` node does not work if you want to
...
access the ``current page`` node. A quick hack save to some typing is to force
