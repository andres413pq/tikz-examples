...
\begin{tikzpicture}[background rectangle/.style={fill=black},show background rectangle] 
  ...
  % spiral too large to for TeX to handle. This is easy to do as the
  ...
  \foreach \i in {1,...,18}
  {
    ...
    \draw[white] (\lastpoint) arc (\startangle : \startangle + 90 : \value{b}/10.0pt) coordinate (\i);
 ...
 % Add some framing for the spiral while not "boxing" it in. Note that
 % to put a square around each turn of the spiral we could have just
 % used the command \draw[white] (\lastpoint) rectangle (\i); after
 % drawing each turn in the loop above.
 \foreach \i in {1,3,...,17}
 {
   \pgfmathsetmacro{\lastpoint}{\i-1}
 ...
 \node(eq) at ($(18) + (2.5,1)$) [white,text width = 2cm,font=\fontsize{8}{8}\selectfont] {
   \begin{displaymath}
     f(n) = \left\{
       \begin{array}{lr}
         0 & \text{~~if } n = 0\\
